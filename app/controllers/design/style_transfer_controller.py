# app/controllers/design/style_transfer_controller.py
...
async def handle(request: Request, container):
    ...
    refimgweight = form_data.get('refimgweight', "")
    ...
    # Validate that the reference image file is present in the form data
    if not reference_image:
        return FormatHelper.format_response(
            "error",
            400,
            "Reference image file is missing",
            None,
            None
        )
    ...
    try:
        response = design_service.style_transfer(
            files=files,
            data=data
        )
    except Exception as e:
        return FormatHelper.format_response(
            "error",
            500,
            f"Failed to transfer style: {str(e)}",
            None,
            None
        )
